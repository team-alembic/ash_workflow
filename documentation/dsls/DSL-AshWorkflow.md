...
| [`until`](#workflow-step-every-until){: #workflow-step-every-until } | `any` |  | Stop firing once this much wall-clock time has passed since the record entered the step. Measured against `repeat_started_at`, never against `state_entered_at`, which every firing resets. Must be strictly longer than `interval`, since equal to it leaves no room to fire even once. |
