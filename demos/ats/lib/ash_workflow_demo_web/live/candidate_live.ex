defmodule AshWorkflowDemoWeb.CandidateLive do
  ...
  defp state_copy(:verifying), do: {"Verifying…", "El Jefe's people are checking you out.", "bg-amber-500"}
  defp state_copy(:review), do: {"Under review", "El Jefe has 30 seconds to decide.", "bg-blue-600"}
  defp state_copy(:hired), do: {"¡HIRED!", "You are El Jefe's new hire. Felicidades.", "bg-emerald-600"}
  defp state_copy(:rejected), do: {"Rejected", "No dice. Better luck next time.", "bg-stone-600"}
  defp state_copy(:auto_rejected), do: {"Auto-rejected", "El Jefe didn't decide in time. Tough town.", "bg-stone-600"}
  defp state_copy(:position_filled), do: {"Position filled", "Someone else got the slot. Vaya con Dios.", "bg-stone-600"}
  defp state_copy(:verification_failed), do: {"Verification failed", "Something broke. Not you. Probably.", "bg-red-600"}
