defmodule AshWorkflow.Changes.RecordEventTest do
  ...
  describe "injection onto action timeouts" do
    ...
    test "an every's action gets RecordEvent and writes its own last-fired column" do
      assert [
               {RecordEvent,
                [
                  triggered_by: :timeout,
                  touch_state_entered_at: false,
                  every_field: :review_reminder_last_fired_at
                ]}
             ] ==
  ...
  describe "atomicity" do
    ...
    test "an every's atomic branch writes only its own field, leaving state_entered_at alone" do
      changeset =
        AshWorkflowTest.EveryUntilWorkflow
        |> Ash.Changeset.for_create(:create, %{title: "atomic every fire"})
        |> Map.put(:action_type, :update)
        |> Map.put(:data, %AshWorkflowTest.EveryUntilWorkflow{state: :waiting})
      ...
      assert {:atomic, _changeset, atomics} =
               RecordEvent.atomic(
                 ...
                 [
                   triggered_by: :timeout,
                   touch_state_entered_at: false,
                   every_field: :waiting_reminder_last_fired_at
                 ],
                 context
               )

      assert %{waiting_reminder_last_fired_at: _now_expr} = atomics
      refute Map.has_key?(atomics, :state_entered_at)
    end
