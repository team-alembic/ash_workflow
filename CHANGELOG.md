...
- `every` can fire at a wall-clock time in the record's own time zone, instead of measuring an `interval` from its last fire. `at ~T[09:00:00]` names the local time, `on [:mon, :tue, :wed, :thu, :fri]` names the days, and `time_zone :candidate_time_zone` names the attribute holding the zone, or takes a literal such as `"Australia/Sydney"`. A per-record zone is the case a global cron cannot express: two records in the same step fire at 09:00 in their own zones, at two different instants. `AshWorkflow.Verifiers.ValidateEvery` rejects an `every` declaring neither. Firing writes the same `last_fired_field` column, and `until` still bounds it against `state_entered_at`.
- `interval` alongside `at` is a stride: how many local days must separate two fires. `on` says which days carry an occurrence and the stride says how many of them to skip, so `interval {14, :days}` with `on [:mon]` is 09:00 every second Monday. A stride must be given in `:days`, and counts local dates rather than elapsed duration — firing at 09:00 writes the last-fired column microseconds after 09:00, so a duration comparison would find the occurrence exactly one stride later ineligible by that margin and slip the fire a week.
- `AshWorkflow.WallClock` computes the occurrences behind `at`. A local time daylight saving skipped still fires that day, at the instant the clock jumped to; a local time that happens twice fires on the first of the two. A missed occurrence fires late rather than being skipped. `AshWorkflow.Scheduler.Precise` arms its timers from it and `AshWorkflow.Calculations.PendingDeadlines` reports the next occurrence from it, so both run on any data layer. `AshWorkflow.Scheduler.Oban` asks the same question in SQL, with an `AT TIME ZONE` fragment reading the zone out of each row, so `at` under that scheduler needs a SQL data layer.
- `time_zone` accepts an expression calculation as well as an attribute or a literal, which is how a record reads its zone off a related one: `calculate :candidate_time_zone, :string, expr(candidate.time_zone)`. The calculation must be expression-based, since the zone is read from the trigger's filter — `AshWorkflow.Verifiers.ValidateEvery` rejects a module calculation, the same check `AshWorkflow.Verifiers.ValidateTimeoutFields` already applies to a timeout's `field`. `AshWorkflow.Scheduler.Precise.Timeline` and `AshWorkflow.Calculations.PendingDeadlines` load a calculated zone before reading it.
- `at` needs a time zone database in the host application, such as `config :elixir, :time_zone_database, Tz.TimeZoneDatabase`. `AshWorkflow.Verifiers.ValidateEvery` checks a literal `time_zone` against it at compile time. A record holding a zone the database does not know arms nothing and reports no pending deadline, rather than raising.
...
- **A bound on `every` with `until`.** `every` fires forever by default; `until` stops it after a fixed amount of wall-clock time:
  ...
  `until` is measured against `state_entered_at` directly, which an `every`'s own firing no longer touches (see above). `AshWorkflow.Verifiers.ValidateEvery` rejects an `until` that is not strictly longer than `interval`, since anything shorter or equal leaves no room to fire even once. Both `AshWorkflow.Scheduler.Oban` and `AshWorkflow.Scheduler.Precise` honour the bound through the same `Work.match` expression every other check already goes through, and `AshWorkflow.Scheduler.Precise.Timeline`'s recovery sweep filters on it too. Reaching the bound only stops the firing; compose a second, ordinary timeout with a `fire_after` equal to the bound for "give up and move on".

### Changed

- **Breaking: `state_entered_at` is no longer writable through an action.** `AshWorkflow.Transformers.AddAttributes` declares it `writable?: false`, so it cannot appear in an action's `accept` list and is rejected as action input. Every deadline on a step measures from this attribute, so a write from outside moved all of them at once, and a `fire_after` timeout could be pushed out of reach by an unrelated update. `AshWorkflow.Changes.RecordEvent` is unaffected: it writes the attribute with `Ash.Changeset.force_change_attribute/3` and, on the atomic path, through the map `atomic/3` returns, and neither goes through `writable?`. A caller that genuinely needs to set it, such as one backfilling imported records, uses `force_change_attribute/3` or the data layer directly. A resource that defines its own `state_entered_at` keeps whatever it declared, since the transformer is a no-op there.
...
- **Breaking: `AshWorkflow.Info.workflow_graph/1`'s `:timeouts` entries drop `:repeat`, and each step's map gains an `:everys` list** of `%{name:, interval:, action:, until:, retry:}` maps, mirroring `:timeouts`.
...
- **An automatic step whose `on_success` is entirely conditional now needs a declared way out.** `AshWorkflow.Verifiers.ValidateWorkflow` rejects, at compile time, a step with no trailing unconditional `on_success` that declares neither `on_error` nor a timeout with `transition_to`. Without one of those, a record whose action succeeds but matches none of the conditions stays in the step and the action is retried on every scheduler cycle forever. A timeout that only names an `action` does not count, since it never changes state. The verifier cannot evaluate arbitrary `Ash.Expr` conditions for logical completeness, so this applies even when a step's conditions are exhaustive by value (`attempts < 3` / `attempts >= 3`, or a boolean and its negation) but declare no unconditional fallback — add `on_error` to fix it.
- **A no-match `on_success` now fails with `AshWorkflow.Errors.NoMatchingRoute` instead of a bare changeset error string.** `AshWorkflow.Changes.ConditionalOnSuccess` names the resource, step and action on the error, and the failure runs down the step's `on_error` the same way any other action failure does. Code matching on the old message text ("No matching on_success route for step ...") should match on `%AshWorkflow.Errors.NoMatchingRoute{}` instead.
